...
if(model_type == "covariate3"){
  ...
  for(i in 1:nsite){
    delta2[i,] <- covs$water_dist[i] - covs$water_dist
  }
  delta2 <- delta2/units::drop_units(site_km)
  ...
  for(i in 1:nsite){
    delta3[i,] <- covs$open_dev[i] - covs$open_dev
  }
  delta3 <- delta3/units::drop_units(site_km)
